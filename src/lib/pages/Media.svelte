<script lang="ts">
	...
	if ($media_type === 'tv') {
		$show_name = movie_details.name;
		$season_count = movie_details.number_of_seasons;
		$show_id = movie_details.id;
	}
...
{#if movie_details.id && trailer_id}
				...
				<div class="xl:col-start-2 xl:col-end-5 flex flex-wrap content-start xl:pl-10">
					<div class="mt-6 xl:mt-0 w-full mb-6 flex flex-wrap">
						<h4 class="w-full xl:text-4xl">
							{'name' in movie_details ? movie_details.name : movie_details.title}
							<span class="text-lg xl:text-4xl text-skin-inverted">
								{'first_air_date' in movie_details
									? movie_details.first_air_date.substring(0, 4)
									: movie_details.release_date
									? movie_details.release_date.substring(0, 4)
									: ''}
							</span>
						</h4>
						<div class="xl:flex">
	...
	{#if $media_type === 'tv'}
		<div class="max-w-7xl mx-auto xl:mt-5 xl:mb-10 bg-skin-secondary xl:pl-5 pt-1 xl:rounded-2xl">
			{#key movie_details}
				<Seasons {movie_details} />
			{/key}
		</div>
	{/if}
{:else}
