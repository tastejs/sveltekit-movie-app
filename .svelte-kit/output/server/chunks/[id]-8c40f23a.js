import { c as create_ssr_component, e as each, a as add_attribute, b as escape, v as validate_component, s as subscribe, f as set_store_value } from "./app-14a936dc.js";
import { a as media_type } from "./store-cc224516.js";
...
async function load({ fetch, page }) {
  const res = await fetch("../api/getMovie", {
    ...
    method: "POST",
    body: JSON.stringify({ media: "person", id: page.params.id })
  });
  ...
  const resp = await fetch("../api/getKnownFor", {
    headers: { "Content-Type": "application/json" },
    method: "POST",
    body: JSON.stringify({ person: page.params.id })
  });
